...
| ARCH-AI-PERCEPTION-001 | P1 | cognitive_runtime | Baseline perception still uses deterministic keyword hints for language/topic/intent and affective fallback. This is acceptable only as a fail-safe, not as the target digital-being perception model. | Backend + AI | ACCEPTED_FIX_NEEDED | Add an AI-assisted structured perception classifier for language, topic, intent, affective cues, and confidence; keep keyword heuristics only as explicit fallback when the provider is unavailable or returns invalid output. |
