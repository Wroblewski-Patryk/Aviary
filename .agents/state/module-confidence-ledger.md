...
| AVIARY-MEMORY-001 | Runtime memory flow | Completed event -> episodic write -> later retrieval -> compressed context -> expression influence -> next episodic write | P0 | VERIFIED | High | `PRJ-1186`; runtime constants `RECENT_MEMORY_LIMIT=6`, `RECENT_MESSAGE_LIMIT=12`, `SEMANTIC_MEMORY_TOP_K=5`, `CONTEXT_TOKEN_BUDGET=2500`; `tests/test_runtime_pipeline.py::test_runtime_recalls_pet_name_from_previous_event_response`; `tests/test_runtime_pipeline.py::test_runtime_applies_concise_response_preference_on_following_turn`; targeted memory/API suite -> `151 passed`; full backend pytest -> `1076 passed`; `/health.memory_retrieval.retrieval_depth_policy` aligned to 6/5; `memory_flow` runtime log includes write status, retrieval counts, retrieved IDs, duration, and token estimate. | Native pgvector SQL/operator proof and provider-backed OpenAI embedding recall remain outside this local slice; current semantic path uses stored embeddings through repository-level cosine similarity and existing deterministic/OpenAI embedding posture. | Add production/provider semantic recall proof when DB and embedding credentials are available. | Backend Builder + QA/Test | 2026-05-13 |
