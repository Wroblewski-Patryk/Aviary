...
- current repo behavior can now also derive a lightweight `goal_progress_score` from active task status mix and recent completions, compare it to the previous reflected score to infer `goal_progress_trend`, persist short goal-level snapshots in `aion_goal_progress`, compress those histories into a semantic `goal_progress_arc`, maintain a lightweight `goal_milestone_state`, emit a lightweight `goal_milestone_transition` when progress crosses meaningful thresholds, sync lightweight `aion_goal_milestone` objects from that state, and use those hints and objects in context, motivation, and planning
- next improvement:
  ...
  - decide how far to push `aion_goal_milestone` before introducing richer milestone semantics such as dependencies, due signals, or explicit completion criteria
