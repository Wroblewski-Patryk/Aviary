...
- current repo behavior can now also derive a lightweight `goal_progress_score` from active task status mix and recent completions, and use that score in context, motivation, and planning
- next improvement:
  ...
  - decide when the runtime should stop treating `goal_progress_score` as a lightweight derived hint and move toward explicit goal-level progress history or scored milestones
