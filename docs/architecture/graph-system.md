...
- The same workflow enforces curated `--gaps` zero-state by failing when
  `query_architecture_graph.py --gaps --format json --fail-on-gaps` returns
  any rows.
- The workflow uploads a gap-audit artifact on every run:
  `architecture-gaps-fast` for automatic gates and
  `architecture-gaps-heavy` for manual heavy runs.
