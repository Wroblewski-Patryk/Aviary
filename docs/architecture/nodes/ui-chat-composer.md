...
- `EVID-UI-CHAT-COMPOSER-PROOF` behavior verified: Chat composer proof refreshed with transcript characterization send and delivered state checks under authenticated chat flow (`web/scripts/chat-transcript-characterization.mjs`). Command: `npm run test:chat-transcript`.
