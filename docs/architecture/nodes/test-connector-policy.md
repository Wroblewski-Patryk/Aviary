...
- `EVID-TEST-CONNECTOR-POLICY-PROOF` test verified: Connector policy tests proof refreshed with operation policy permission-gate and guardrail snapshot contracts (`backend/tests/test_connector_policy.py`). Command: `python -m pytest -q tests/test_connector_policy.py`.
