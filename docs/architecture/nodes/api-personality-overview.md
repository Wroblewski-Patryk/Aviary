...
- `EVID-API-PERSONALITY-OVERVIEW-PROOF` behavior verified: Personality overview API proof refreshed with authenticated user learned-state overview and recent activity internal-row filtering checks (`backend/tests/test_api_routes.py`). Command: `python -m pytest -q tests/test_api_routes.py::test_app_personality_overview_uses_authenticated_user tests/test_memory_repository.py::test_memory_repository_ignores_internal_rows_when_counting_unanswered_proactive_and_recent_activity`.
