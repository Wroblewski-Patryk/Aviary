...
- [[feat-telegram|FEAT-TELEGRAM]] -> `depends_on`: Telegram flow depends on delivery routing transport boundaries
...
- `EVID-SERVICE-DELIVERY-ROUTER-PROOF` behavior verified: Delivery router service proof refreshed with Telegram channel success missing-chat-id failure and transport segmentation contract coverage (`backend/tests/test_delivery_router.py`). Command: `python -m pytest -q tests/test_delivery_router.py`.
