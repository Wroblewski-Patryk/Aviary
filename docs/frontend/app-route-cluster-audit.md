...
| `/insights` | 5260 | Module-style overview route using shared note/stat patterns | Good candidate for shared side-panel/row extraction with automations |
| `/automations` | 5357 | Module-style route mixed with health-derived scheduler posture | Good candidate for shared side-panel/row extraction with insights |
...
`PRJ-995` should extract shared module route side-panel/row presentation for
`/insights` and `/automations`:

- add a small shared component that preserves route-specific class prefixes
- move only side-panel and row chrome
- keep derived row data in `App()`
