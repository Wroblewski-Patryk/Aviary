...
- Extra responsibility: for agent tools, MCP bridges, provider actions, and
  command routes, keep `docs/operations/approval-aware-agent-command-flow.md`
  adapted and enforced.
...
- Agents must respect the project's adapted
  `docs/governance/agent-runtime-contract.md` when one exists.
- External-memory and command-capable agents must use API-only access, scoped
  credentials, startup handshakes, and fail-closed command behavior.
